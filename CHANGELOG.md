...
### Added
- **장비 IP 주소 수정** — 설정 드로어에서 장비 IP 주소를 직접 편집할 수 있습니다. MAC 주소는 비활성(읽기 전용)으로 표시됩니다.

### Changed
- **설정 드로어 정보 카드 제거** — 장비 설정 드로어 상단의 읽기 전용 정보 카드(IP·MAC·프로토콜·상태·모델·업타임)가 제거됩니다. 동일 정보는 관제 드로어에서 확인할 수 있습니다.
- **관제 드로어 기본정보 레이아웃 변경** — 기본정보 탭의 장비 정보가 2열 박스 그리드에서 단순 수직 리스트(라벨→값)로 변경됩니다.

### Fixed
- **ip_addr 업데이트 유효성 검증** — `DeviceUpdate` 스키마에 IPv4 형식 검증이 없어 임의의 문자열이 저장될 수 있던 문제를 수정합니다.
